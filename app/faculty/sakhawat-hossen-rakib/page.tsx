...
import { Mail, Phone, MapPin, Star, ArrowLeft, Award, Users, FileText, Calendar, Trophy, GraduationCap, Building, ExternalLink } from 'lucide-react';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `${member.name} - ${member.title} | UAP EEE Department`,
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
                ...
                <div className="space-y-4">
                  {member.academicExperience.map((exp, index) => (
                    <div key={index} className="border-l-4 border-indigo-200 pl-4">
                      ...
                      <p className="text-indigo-600">{exp.institute}</p>
                ...
                <div className="space-y-4">
                  {member.researchExperience.map((research, index) => (
                    <div key={index} className="border-l-4 border-green-200 pl-4">
                      ...
                      <p className="text-gray-500 text-sm">{research.period}</p>
                      {research.supervisor && (
                        <p className="text-indigo-600">Supervisor: {research.supervisor}</p>
                      )}
                      {research.institution && (
                        <p className="text-indigo-600">{research.institution}</p>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {member.academicScholarships.map((scholarship, index) => (
                    <div key={index} className="border-l-4 border-yellow-200 pl-4">
                      <h3 className="font-semibold text-gray-900">{scholarship.award}</h3>
                      <p className="text-gray-500 text-sm">{scholarship.period}</p>
                      {scholarship.details && (
                        <p className="text-gray-700 text-sm mt-2">{scholarship.details}</p>
                      )}
                    </div>
                  ))}
                </div>
                  ...
                  {member.recentPublications.slice(0, 10).map((publication, index) => (
                    <div key={index} className="p-4 bg-gray-50 rounded-lg border-l-4 border-blue-200">
                      <div className="flex items-start justify-between mb-2">
                        <Badge variant={publication.type === 'Journal' ? 'default' : 'secondary'} className="mb-2">
                          {publication.type}
                        </Badge>
                      ...
                      <p className="text-sm text-indigo-600">{publication.venue}</p>
                      {publication.impactFactor && (
                        <p className="text-xs text-green-600 mt-1">Impact Factor: {publication.impactFactor}</p>
                      )}
                      {publication.doi && (
                        <p className="text-xs text-gray-500 mt-1">DOI: {publication.doi}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {/* Publications - Old Structure (Fallback) */}
            {(member.recentPublications?.journalPapers?.length > 0 || member.recentPublications?.conferencePapers?.length > 0) && (
              <div className="bg-white rounded-2xl shadow-lg p-8 mt-8">
                <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
                  <FileText className="w-6 h-6 mr-3 text-indigo-600" />
                  Recent Publications
                ...
                {member.recentPublications.conferencePapers?.length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Conference Papers</h3>
                    <div className="space-y-3">
                      {member.recentPublications.conferencePapers.slice(0, 5).map((paper, index) => (
                        <div key={index} className="p-4 bg-gray-50 rounded-lg">
                          <h4 className="font-medium text-gray-900 mb-1">{paper.title}</h4>
                          <p className="text-sm text-gray-600">{paper.authors}</p>
                          <p className="text-sm text-indigo-600">{paper.conference} ({paper.year})</p>
                        </div>
                      ))}
                    </div>
                  </div>
