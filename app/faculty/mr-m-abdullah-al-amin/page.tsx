                    ...
                    <div key={index} className="p-4 bg-gray-50 rounded-lg border-l-4 border-indigo-500">
                      <div className="flex items-start justify-between mb-2">
                        <Badge 
                          ...
                          className={publication.type === 'Journal' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}
                        >
                          {publication.type}
                        </Badge>
                        <span className="text-sm font-medium text-gray-500">{publication.year}</span>
                      </div>
                      <h4 className="font-medium text-gray-900 mb-2 leading-tight">{publication.title}</h4>
                      <p className="text-sm text-gray-600 mb-1">{publication.authors}</p>
                      <p className="text-sm text-indigo-600 font-medium">{publication.venue}</p>
                      {publication.impactFactor && (
                        <p className="text-xs text-gray-500 mt-1">Impact Factor: {publication.impactFactor}</p>
                      )}
                      {publication.doi && (
