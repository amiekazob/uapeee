...
console.log('Converting faculty pages to independent designs...');
console.log('Found faculty IDs:', facultyIds.length);

// Template for independent faculty page design
const independentPageTemplate = (facultyId) => `import { facultyMembers } from '@/lib/faculty-data';
...
facultyIds.forEach(facultyId => {
  ...
  if (!fs.existsSync(facultyDir)) {
    console.log(`Skipped: ${facultyId} (directory doesn't exist)`);
    skipped++;
    return;
  }
  ...
  if (fs.existsSync(pageFile)) {
    const backupFile = path.join(facultyDir, 'page.tsx.backup');
    fs.copyFileSync(pageFile, backupFile);
  }
  
  // Write the new independent design
  fs.writeFileSync(pageFile, independentPageTemplate(facultyId));
  console.log(`Converted: ${facultyId}`);
  converted++;
});

console.log(`\nConversion Summary: Converted ${converted} pages, Skipped ${skipped} pages`);
console.log('All faculty pages now have independent, customizable designs!');